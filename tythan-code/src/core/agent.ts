...
export class Agent {
  ...
  // Set for the duration of runTurn; stop() aborts the in-flight model call
  // and ends the turn at the next safe point (a point where history is
  // consistent — never between a tool_use and its tool_result).
  ...
  /** True while runTurn is processing a user message. */
  get running(): boolean {
    return this.turnAbort !== undefined;
  }

  /** Stop the current turn: aborts the in-flight model request and skips any
   * remaining tool rounds. Safe no-op when nothing is running. */
  stop(): void {
  ...
  private async executeTool(name: string, toolInput: Record<string, unknown>): Promise<{ output: string; isError: boolean }> {
    ...
    try {
      ...
      // Unexpected errors (e.g. a raw EACCES/ENOTDIR from fs inside
      // writeFile) must also come back as an error *result*, not propagate:
      // the model has already emitted a tool_use block into history, and a
      // provider like Anthropic rejects every subsequent request in a
      // conversation where a tool_use has no matching tool_result — one
      // throw here would otherwise wedge the whole session.
      const e = err as Error;
      return { output: `Tool failed unexpectedly: ${e?.message ?? String(err)}`, isError: true };
    }
  }
      ...
      while (true) {
        ...
        try {
          ...
          if (abort.signal.aborted) {
            ...
            // error. Nothing was appended to history for this call, so the
            // conversation is still consistent — report and end quietly.
            this.sink.info("generation stopped");
            return;
          }
          throw err;
        }
        ...
        for (const call of result.toolCalls) {
          ...
          const { output, isError } = abort.signal.aborted
            ? { output: "Skipped: the user stopped this turn.", isError: true }
            : await this.executeTool(call.name, call.input);
        ...
        if (abort.signal.aborted) {
          // Stop *after* pairing every tool_use with a tool_result, so the
          // provider-side conversation stays valid for the next turn.
          this.sink.info("generation stopped");
          return;
        }
      }
