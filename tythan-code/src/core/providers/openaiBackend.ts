...
export class OpenAIBackend extends Backend {
  ...
  private async createStream(
    ...
    if (this.usageSupported === false) {
      return this.client.chat.completions.create(base, { signal });
    }
    try {
  ...
  async streamTurn(
    ...
    const stream = await this.createStream(fullMessages, tools, signal);
  ...
  override async completeText(system: string, userText: string, options?: CompleteTextOptions): Promise<string> {
    // Capped like AnthropicBackend.completeText — the default keeps a
    // verbose/non-compliant model from returning an unbounded completion
    // that defeats the point of compacting; callers with bigger needs
    // (inline edit of a large selection) raise it explicitly.
