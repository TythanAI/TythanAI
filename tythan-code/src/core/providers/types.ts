...
export interface CompleteTextOptions {
  /** Cap on the completion's output tokens (default: a small summary-sized cap). */
  maxTokens?: number;
  /** Abort the underlying HTTP request (e.g. user cancelled the action). */
  signal?: AbortSignal;
}
...
export abstract class Backend {
   ...
   * `signal`, when provided, aborts the in-flight HTTP request. */
  abstract streamTurn(
  ...
  /** One-shot, tool-free completion — used for compaction summaries, inline
   * tab-completion and inline edits. Must be overridden by any backend that
   * wants to support those features. */
  async completeText(_system: string, _userText: string, _options?: CompleteTextOptions): Promise<string> {
    throw new Error(`${this.name} backend does not support one-shot completions`);
  }
}
