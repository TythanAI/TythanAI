...
describe("AnthropicBackend.completeText", () => {
  ...
  it("defaults max_tokens to the summary cap but honors an explicit override", async () => {
    const seen: number[] = [];
    const createFn = vi.fn(async (params: { max_tokens: number }) => {
      seen.push(params.max_tokens);
      return { content: [{ type: "text", text: "ok" }] };
    });
    const backend = makeBackend(makeClient(() => new FakeMessageStream([], {}), createFn));
    await backend.completeText("sys", "text");
    await backend.completeText("sys", "text", { maxTokens: 4321 });
    expect(seen).toEqual([2000, 4321]);
  });

  it("passes the abort signal through to the SDK", async () => {
    const controller = new AbortController();
    const captured: Array<AbortSignal | undefined> = [];
    const createFn = vi.fn(async (_params: unknown, options?: { signal?: AbortSignal }) => {
      captured.push(options?.signal);
      return { content: [{ type: "text", text: "ok" }] };
    });
    const backend = makeBackend(makeClient(() => new FakeMessageStream([], {}), createFn));
    await backend.completeText("sys", "text", { signal: controller.signal });
    expect(captured).toEqual([controller.signal]);
  });
});

describe("AnthropicBackend.streamTurn abort signal", () => {
  it("hands the signal to messages.stream as a request option", async () => {
    const controller = new AbortController();
    const captured: Array<AbortSignal | undefined> = [];
    const client = {
      messages: {
    ...
    const backend = makeBackend(client);
    await backend.streamTurn([{ role: "user", content: "hi" }], "sys", [], makeSink(), controller.signal);
    expect(captured).toEqual([controller.signal]);
  });
});
